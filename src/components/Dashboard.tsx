...
export function Dashboard({ snapshot }: { snapshot: MorningMarketSnapshot }) {
  ...
  const liveCount = snapshot.indicators.filter((item) => item.source.startsWith("FRED") || item.source.startsWith("Twelve Data") || item.source.startsWith("한국은행 ECOS")).length;
  const hasLiveDisclosures = snapshot.disclosures.length === 0 || snapshot.disclosures.some((item) => item.sourceUrl.includes("rcpNo="));
  ...
  return <main>
    ...
    <div id="top" className="page-shell">
      <section className="hero"><div className="hero-copy"><div className="mock-label live-label"><span></span> 실데이터 · 일부 대체 데이터</div><p className="date-line">{date}</p><h1>출근길<br />시장 브리핑</h1><p className="hero-subtitle">국내 증시 개장 전, 밤사이 시장의 핵심 신호를 5분 안에 차분하게 확인하세요.</p></div><div className="briefing-status"><div className="status-orbit"><span className="status-core">수집</span></div><div><p>최근 서버 수집</p><strong>{snapshot.generatedAt.slice(11, 16)} KST</strong><span className="freshness"><i></i> 실데이터 {liveCount}개 · 대체 {snapshot.indicators.length - liveCount}개</span></div></div></section>
